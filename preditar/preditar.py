#!/usr/bin/python2.7 -W ignore::DeprecationWarning
# -*- coding: utf-8 -*-
#home_folder = "/home/user/naeem/scikit/new/preditar/"
home_folder = "../preditar/"
...
sys.path.insert(1,'/opt/RDKit_2013_06_1')
import numpy as np
from rdkit import Chem
from rdkit.Chem import AllChem
from rdkit import DataStructs
from sklearn.externals import joblib
sys.path.insert(0, '/opt/RDKit_2013_06_1/rdkit')
import PrediTAREmail
#### HTML ####
...
parser.add_argument('-e', '--email', help='Email address/s.', required=True)
...
parser.add_argument('-p', '--prob', help='probability cut-off for prediction', default=0.75, type=float)
parser.add_argument('-m', '--mode', help='enter 3 for 3D classifier, 2 for 2D classifier and 1 for combo', default='3d', type=str, choices=['3d', '2d', 'combo'])
...
if mode=='3d':
   f1 = open("./UserData/"+outF+"_PrediTar_3d.csv","wb")
   writer = csv.writer(f1, delimiter=";", quotechar=' ')
   title_line = ["lig_name", "PDB_id", "het_id", "probability", "link"]
   writer.writerow(title_line)
   String = String + PrediTAREmail.Print3DHeader() + "\n"
   for lig, pred in r3.iteritems():
      writer.writerow([lig])
      for pdb, prob3 in pred.iteritems():
         pro, het = pdb.split('-')
         link = 'http://www.rcsb.org/pdb/explore.do?structureId=%s' %(pro)
         writer.writerow(['', pro, het, prob3, link])
         String = String + PrediTAREmail.TableDataGeneratorFor3D(lig, pro, het, str(prob3)) + "\n"
   f1.close()
   #print "predicted targets are saved in `%s_PrediTars_3d.csv` file" %(outF)


if mode=='2d':
   f2 = open("./UserData/"+outF+"_PrediTar_2d.csv","wb")
   writer = csv.writer(f2, delimiter=";", quotechar=' ')
   title_line = ["lig_name", "uniprot_acc", "probability"]
   writer.writerow(title_line)
   String = String + PrediTAREmail.Print2DHeader() + "\n"
   for lig, pred in r2.iteritems():
      writer.writerow([lig])
      for uni, prob2 in pred.iteritems():
         writer.writerow(['', uni, prob2])
         String = String + PrediTAREmail.TableDataGeneratorFor2D(lig, uni, str(prob2)) + "\n"
   f2.close()
   #print "predicted targets are saved in `%s_PrediTar_2d.csv` file" %(outF)
   
if mode=='combo':
   f2 = open("./UserData/"+outF+"_PrediTar_combo.csv","wb")
   writer = csv.writer(f2, delimiter=";", quotechar=' ')
   title_line = ["lig_name", "PDB_id",  "het_id", "prob3", "uniprot_acc", "prob2"]
   writer.writerow(title_line)
   String = String + PrediTAREmail.PrintComboHeader() + "\n"
   for lig, pred in r3.iteritems():
      writer.writerow([lig])
      for pdb, prob3 in pred.iteritems():
         pro, het = pdb.split('-')
         ...
         String = String + PrediTAREmail.TableDataGeneratorForCombo(lig, pro, het, str(prob3), uni, str(prob2)) + "\n"
   f2.close()
   #print "predicted targets are saved in `%s_PrediTar_combo.csv` file" %(outF)   

annotTP.close()

##########################3 For Mailing our Results.... #################
send_from = 'noreply@pglab'
send_to = [email]
subject = '[PrediTar] '+ 'Your PrediTar results for job name: ' + outF

PrediTAREmail.CallHeader()
text = PrediTAREmail.CallHeader() + "\n" + String + "\n"+ PrediTAREmail.CallFooter() 
part2 = MIMEText(text, 'html')
files = ['./UserData/'+outF+'_PrediTar_'+mode+'.csv']

PrediTAREmail.send_mail(send_from, send_to, subject, text, files, server="localhost")
Khaled = "This is India."
import json
print text
#annotTN.close()
